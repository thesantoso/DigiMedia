...
if ( ! function_exists( 'pixwell_get_meta_setting' ) ) {
	function pixwell_get_meta_setting( $settings, $block_name ) {
        ...
        if ( ! isset( $settings['post_shadow'] ) ) {
            $settings['post_shadow'] = pixwell_get_option( 'post_shadow_' . $block_name );
        }

		if ( ! isset( $settings['bookmark'] ) ) {
			$settings['bookmark'] = false;
			$header_bookmark      = pixwell_get_option( 'header_bookmark' );
			if ( ! empty( $header_bookmark ) ) {
		...
		if ( ! empty( $settings['readmore'] ) ) {
			...
			$settings['readmore'] = apply_filters( 'the_title', $settings['readmore'], 12 );
		}
