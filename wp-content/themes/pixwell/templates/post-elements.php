...
if ( ! function_exists( 'pixwell_post_title' ) ) {
	function pixwell_post_title( $h_tag = 'h2', $bookmark = false, $classes = '' ) {
        ...
        <a class="p-url" href="<?php echo esc_url( get_permalink() ); ?>" rel="bookmark"
           title="<?php echo wp_strip_all_tags( $post_title ); ?>"><?php
			if ( ! empty( $post_title ) ) {
				the_title();
			} else {
				echo get_the_date();
...
if ( ! function_exists( 'pixwell_post_readmore' ) ) {
	function pixwell_post_readmore( $settings = array() ) {
        ...
        <a class="btn p-link"
           href="<?php echo get_permalink(); ?>"><span><?php echo esc_html( $settings['readmore'] ); ?></span><i
        ...
        <span class="meta-info-el meta-info-date">
			...
			<?php if ( ! empty( $timestamp ) ) : ?>
                <abbr class="date published"
                      title="<?php echo get_the_date( DATE_W3C, $post_id ); ?>"><?php echo sprintf( pixwell_translate( 'ago' ), human_time_diff( $timestamp ) ); ?></abbr>
			<?php else : ?>
                <abbr class="date published"
                      title="<?php echo get_the_date( DATE_W3C, $post_id ); ?>"><?php echo get_the_date( '', $post_id ); ?></abbr>
			<?php endif; ?>
		</span>
		<?php
	}
        ...
        <span class="meta-info-el meta-info-update">
			...
			<?php if ( ! empty( $timestamp ) ) : ?>
                <abbr class="date date-updated"
                      title="<?php echo get_the_modified_date( DATE_W3C, $post_id ); ?>"><?php echo sprintf( pixwell_translate( 'ago' ), human_time_diff( $timestamp, current_time( 'timestamp' ) ) ); ?></abbr>
			<?php else : ?>
                <abbr class="date date-updated"
                      title="<?php echo get_the_modified_date( DATE_W3C, $post_id ); ?>"><?php echo get_the_modified_date( '', $post_id ); ?></abbr>
			<?php endif; ?>
		</span>
		<?php
	}
...
if ( ! function_exists( 'pixwell_entry_meta_authors' ) ) {
	 ...
	 */
	function pixwell_entry_meta_authors( $author_data = array() ) {
			...
			<?php if ( ! empty( $author_avatar ) ): ?>
                <span class="meta-avatar">
					<?php foreach ( $author_data as $author ) :
						$author_id = $author->ID; ?>
						<?php echo get_avatar( get_the_author_meta( 'user_email', $author_id ), 36, '', get_the_author_meta( 'display_name', $author_id ) ); ?>
					<?php endforeach; ?>
			    </span>
			<?php else :
				$meta_author_text = pixwell_get_option( 'meta_author_text' );
...
if ( ! function_exists( 'pixwell_single_entry_meta_authors' ) ) {
	 ...
	 * @param array $author_data
	 */
	function pixwell_single_entry_meta_authors( $author_data = array() ) {
		$label            = pixwell_get_option( 'single_post_author_meta_label' );
		$meta_author_text = pixwell_get_option( 'meta_author_text' ); ?>
...
if ( ! function_exists( 'pixwell_post_meta_cat' ) ) {
	function pixwell_post_meta_cat( $primary = true ) {
		...
		if ( empty( $primary_category ) ) :
			if ( array( $categories ) ) : ?>
                <span class="meta-info-el meta-info-cat">
					<?php foreach ( $categories as $category ) : ?>
                        <a class="cat-<?php echo esc_attr( $category->term_id ); ?>"
            ...
            <span class="meta-info-el meta-info-cat">
				<a class="cat-<?php echo esc_attr( $primary_category ); ?>"
                   href="<?php echo esc_url( get_category_link( $primary_category ) ); ?>"><?php echo esc_html( $primary_category_name ); ?></a>
			</span>
		<?php endif;
	}
}
			...
			<em class="meta-label"><?php echo pixwell_translate( 'tags' ) . ' '; ?></em>
				<?php foreach ( $all_tags as $tag ) : ?>
                    <a href="<?php echo esc_url( get_tag_link( $tag->term_id ) ); ?>"
                       rel="tag"><?php echo esc_attr( $tag->name ); ?></a>
				<?php endforeach; ?>
			</span>
		<?php endif;
	}
			...
			<a href="<?php echo get_comments_link() ?>">
				<?php if ( ! empty( $meta_comment_icon ) ) : ?>
					...
					if ( 0 == $total_comments ) {
						...
						echo sprintf( pixwell_translate( 'comments' ), pixwell_show_over_k( $total_comments ) );
					} ?>
				<?php
				else :
					if ( 0 == $total_comments ) {
			...
			<a href="<?php echo get_permalink() ?>" title="<?php esc_attr( get_the_title() ); ?>">
				<?php if ( ! empty( $meta_view_icon ) ) : ?>
					...
					if ( 1 == $total_view ) {
						...
						echo sprintf( pixwell_translate( 'views' ), pixwell_show_over_k( $total_view ) );
					} ?>
				<?php
				else :
					if ( 1 == $total_view ) {
...
if ( ! function_exists( 'pixwell_post_meta_custom' ) ) {
	function pixwell_post_meta_custom( $post_id = '' ) {
        ...
        <span class="meta-info-el meta-info-custom">
            ...
            <i class="rbi <?php echo esc_attr( $meta_custom_icon ); ?>"></i><?php echo esc_html( $meta_entry ); ?>
			<?php
			else :
...
if ( ! function_exists( 'pixwell_post_thumb' ) ) {
	function pixwell_post_thumb( $size = 'full', $classes = '', $format = true, $edit_link = true ) {
        ...
        <a class="p-flink" href="<?php echo get_permalink(); ?>" title="<?php echo wp_strip_all_tags( get_the_title() ); ?>">
            <span class="<?php echo esc_attr( $class_name ); ?>"><?php the_post_thumbnail( $size ); ?></span>
        </a>
	...
	function pixwell_post_cat_info( $settings ) {
        ...
        <aside class="<?php echo esc_attr( $class_name ); ?>">
			<?php if ( ! empty( $settings['cat_info'] ) ) : ?>
				<?php if ( empty( $primary_category ) || empty ( $primary_category_name ) ) :
...
if ( ! function_exists( 'pixwell_post_review_info' ) ) {
	function pixwell_post_review_info( $settings ) {
		...
		if ( empty( $total_stars ) ) {
			$total_stars = pixwell_add_total_review( get_the_ID() );
		}

		$review_meta = rb_get_meta( 'review_meta' ); ?>
        <aside class="p-review-info is-light-text">
            <i class="rbi rbi-star-full"></i>
            <span class="meta-total h4"><?php echo esc_html( $total_stars ); ?></span>
        ...
        <aside class="p-meta-sponsor">
            <div class="sponsor-inner">
                <i class="rbi rbi-bell sponsor-icon"></i><span
                        class="sponsor-label"><?php echo apply_filters( 'the_title', $label, 12 ); ?></span>
                <a class="sponsor-link" href="<?php echo esc_url( $sponsor_url ); ?>" target="_blank"
                   rel="noopener nofollow"><?php echo esc_html( $sponsor_name ); ?></a>
            </div>
        </aside>
		<?php
		return ob_get_clean();
	...
	function pixwell_post_meta_shop_post() {
        ...
        <aside class="p-meta-info">
			<span class="meta-info-el meta-shop-post">
				<a class="shop-post-link" href="<?php echo get_permalink() . '#shopthepost'; ?>"><i
                            class="rbi rbi-shop-bag shop-post-icon"></i><span
                            class="shop-post-label"><?php echo apply_filters( 'the_title', $label, 10 ); ?></span></a>
			</span>
        </aside>
		<?php
		return ob_get_clean();
