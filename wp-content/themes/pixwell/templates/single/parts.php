...
if ( ! function_exists( 'pixwell_single_featured_image' ) ) :
	function pixwell_single_featured_image( $size = '', $classes = 'single-feat' ) {
		...
		if ( ! defined( 'PIXWELL_CORE_VERSION' ) ) : ?>
			<div class="single-feat"><?php the_post_thumbnail( $size ); ?></div>
			<?php return false;
		endif; ?>
		<div class="<?php echo esc_attr( $classes ); ?>">
			<div class="<?php echo esc_attr( $class_name ); ?>"><?php the_post_thumbnail( $size ); ?></div>
					...
					<div class="review-footer">
						...
						if ( ! empty( $review_button ) ) : ?>
							<div class="review-action">
								<a class="review-btn" href="<?php echo esc_html( $review_destination ); ?>" target="_blank" rel="nofollow noreferrer"><?php echo esc_html( $review_button ); ?></a>
							</div>
						<?php endif;
		...
		<div class="single-meta-bottom p-meta-info">
			...
			if ( ! empty( $updated ) ) : ?>
				<div class="updated-info meta-info-el meta-info-author">
					<em class="meta-label"><?php echo pixwell_translate( 'updated' ); ?></em><span class="updated-date"><?php echo get_the_modified_date( '', get_the_ID() ); ?></span>
				</div>
			<?php endif;
			?></div>
	<?php
        ...
        if (function_exists('get_multiple_authors')) {
            ...
            if (is_array($author_data) && count($author_data) > 1) { ?>
                <span class="single-meta-avatar meta-avatar">
                    ...
                    <?php echo get_avatar(get_the_author_meta('user_email', $author_id), 60, '', get_the_author_meta('display_name', $author_id)); ?>
                    </a>
                    <?php endforeach; ?>
                </span>
                <?php return false;
...
if ( ! function_exists( 'pixwell_rbc_related' ) ) :
	function pixwell_rbc_related( $attrs ) {
		...
		ob_start(); ?>
		<aside class="<?php echo esc_attr( $classes ); ?>">
			<h6 class="rb-related-header h3"><?php echo apply_filters( 'widget_title', $settings['title'], 12 ) ?></h6>
				...
				<?php while ( $query_data->have_posts() ) :
					...
					<div class="rb-col-t6 rb-col-m12 rb-p15-gutter">
						<div class="rb-related-el">
							...
							<?php endif;
							the_title( '<h6 class="related-title">', '</h6>' ); ?>
