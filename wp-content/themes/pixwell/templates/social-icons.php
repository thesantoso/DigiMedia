...
if ( ! function_exists( 'pixwell_render_social_icons' ) ) :
	function pixwell_render_social_icons( $data = array(), $new_tab = true, $custom = true ) {
		...
		if ( ! empty( $telegram ) ) {
			$str .= '<a class="social-link-telegram" title="' . esc_attr__( 'Telegram', 'pixwell' ) . '" href="' . esc_url( $telegram ) . '" ' . $new_tab . '><i class="rbi rbi-telegram"></i></a>';
		}
		if ( ! empty( $whatsapp ) ) {
		...
		if ( true === $custom ) {
			...
			if ( ! empty( $social_1_url ) && ! empty( $social_1_name ) ) {
				$str .= '<a class="social-link-custom social-link-1 social-link-' . esc_attr( $social_1_name ) . '" title="' . esc_attr( $social_1_name ) . '" href="' . esc_url( $social_1_url ) . '" ' . $new_tab . '><i class="' . esc_attr( $social_1_icon ) . '"></i></a>';
			}
			if ( ! empty( $social_2_url ) && ! empty( $social_2_name ) ) {
				$str .= '<a class="social-link-custom social-link-2 social-link-' . esc_attr( $social_2_name ) . '" title="' . esc_attr( $social_2_name ) . '" href="' . esc_url( $social_2_url ) . '" ' . $new_tab . '><i class="' . esc_attr( $social_2_icon ) . '"></i></a>';
			}
			if ( ! empty( $social_3_url ) && ! empty( $social_3_name ) ) {
				$str .= '<a class="social-link-custom social-link-3 social-link-' . esc_attr( $social_3_name ) . '" title="' . esc_attr( $social_3_name ) . '" href="' . esc_url( $social_3_url ) . '" ' . $new_tab . '><i class="' . esc_attr( $social_3_icon ) . '"></i></a>';
			}
		}
