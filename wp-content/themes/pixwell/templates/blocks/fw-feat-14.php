...
if ( ! function_exists( 'pixwell_rbc_fw_feat_14' ) ) {
	function pixwell_rbc_fw_feat_14( $attrs ) {
		...
		$settings['classes']       = 'fw-block fw-feat-14 none-margin';
		...
		if ( ! $query_data->have_posts() ) :
			...
			echo '<div class="rb-owl slider-feat-14 per-load">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				pixwell_post_list_6( $settings );
			endwhile;
			echo '</div>';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_14' ) ) {
	function pixwell_register_fw_feat_14( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_14',
			'title'       => esc_html__( 'Deal Slider', 'pixwell' ),
			...
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-14.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_14', 12 );