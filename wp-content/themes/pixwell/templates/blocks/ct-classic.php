 ...
 * ct classic
 */
if ( ! function_exists( 'pixwell_rbc_ct_classic' ) ) {
	function pixwell_rbc_ct_classic( $attrs ) {
		...
		$settings['classes'] = 'ct-block ct-classic';
...
/** content classic listing */
if ( ! function_exists( 'pixwell_rbc_ct_classic_listing' ) ) :
	function pixwell_rbc_ct_classic_listing( $settings, $query_data ) {
			...
			while ( $query_data->have_posts() ) :
				...
				if ( ! empty( $settings['infeed_ad'] ) && ! wp_doing_ajax() ) :
					if ( ( ! empty( $settings['ad_pos_1'] ) && intval( $settings['ad_pos_1'] ) == $counter ) || ( ! empty( $settings['ad_pos_2'] ) && intval( $settings['ad_pos_2'] ) == $counter ) ) {
						echo '<div class="rb-col-m12 infeed-outer">';
 ...
 */
if ( ! function_exists( 'pixwell_register_ct_classic' ) ) {
	function pixwell_register_ct_classic( $blocks ) {
		...
		$blocks[] = array(
			...
			'description' => esc_html__( 'Display your posts as a classic layout in the content section.', 'pixwell' ),
			'section'     => 'content',
...
add_filter( 'rbc_add_block', 'pixwell_register_ct_classic', 100 );
