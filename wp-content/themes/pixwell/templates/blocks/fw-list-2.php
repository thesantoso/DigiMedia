 ...
 * fullwidth list 2
 */
if ( ! function_exists( 'pixwell_rbc_fw_list_2' ) ) {
	function pixwell_rbc_fw_list_2( $attrs ) {
		...
		$settings['classes']         = 'fw-block fw-list-2';
 ...
 * fw list 2 listing
 */
if ( ! function_exists( 'pixwell_rbc_fw_list_2_listing' ) ) :
	function pixwell_rbc_fw_list_2_listing( $settings, $query_data ) {
			...
			while ( $query_data->have_posts() ) :
				...
				if ( ! empty( $settings['infeed_ad'] ) && ! wp_doing_ajax() ) :
					if ( ( ! empty( $settings['ad_pos_1'] ) && intval( $settings['ad_pos_1'] ) == $counter ) || ( ! empty( $settings['ad_pos_2'] ) && intval( $settings['ad_pos_2'] ) == $counter ) ) {
						echo '<div class="p-outer rb-p20-gutter rb-col-m12 rb-col-d6 infeed-outer">';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_list_2' ) ) {
	function pixwell_register_fw_list_2( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_list_2',
			'title'       => esc_html__( 'List 2', 'pixwell' ),
			'description' => esc_html__( 'Display your posts as a list (2 columns) in the fullwidth section.', 'pixwell' ),
			'section'     => 'fullwidth',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_list_2', 31 );
