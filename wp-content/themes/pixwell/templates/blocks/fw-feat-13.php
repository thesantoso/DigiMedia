 ...
 * fullwidth feat 13
 */
if ( ! function_exists( 'pixwell_rbc_fw_feat_13' ) ) {
	function pixwell_rbc_fw_feat_13( $attrs ) {
		...
		if ( $settings['posts_per_page'] > 6 ) {
			$settings['posts_per_page'] = 6;
		}

		$settings['classes']       = 'fw-block fw-feat-13 none-margin';
		...
		if ( ! $query_data->have_posts() ) :
			...
			echo '<div class="rb-owl slider-feat-13 per-load">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				pixwell_post_overlay_9( $settings );
			endwhile;
			echo '</div>';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_13' ) ) {
	function pixwell_register_fw_feat_13( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_13',
			'title'       => esc_html__( 'Gadgets Slider', 'pixwell' ),
			...
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-13.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
				...
				array(
					...
					'description' => esc_html__( 'Select number of posts per page (total posts to show). Maximum value is 6.', 'pixwell' ),
					'default'     => 4
				),
				array(
					'name'        => 'offset',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_13', 11 );