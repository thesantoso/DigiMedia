...
if ( ! function_exists( 'pixwell_rbc_fw_grid_4' ) ) {
	function pixwell_rbc_fw_grid_4( $attrs ) {
		...
		$settings['classes']         = 'fw-block fw-grid-4';
		$settings['content_classes'] = 'rb-n10-gutter';
 ...
 * fw grid 4 listing
 */
if ( ! function_exists( 'pixwell_rbc_fw_grid_4_listing' ) ) :
	function pixwell_rbc_fw_grid_4_listing( $settings, $query_data ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			...
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				echo '<div class="rb-col-m12 rb-col-5d rb-p10-gutter">';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_grid_4' ) ) {
	function pixwell_register_fw_grid_4( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_grid_4',
			'title'       => esc_html__( 'Grid 4', 'pixwell' ),
			'description' => esc_html__( 'Display your posts as a grid (5 columns + with/without popular style) in the fullwidth section.', 'pixwell' ),
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-grid-4.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
				...
				array(
					...
					'type'        => 'select',
					'tab'         => 'design',
					'title'       => esc_html__( 'Popular Style', 'pixwell' ),
					'description' => esc_html__( 'Enable or disable popular style (post number counter) for this block layout.', 'pixwell' ),
					'options'     => array(
						'0' => esc_html__( '-Disable-', 'pixwell' ),
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_grid_4', 24 );
