 ...
 * fullwidth list 3
 */
if ( ! function_exists( 'pixwell_rbc_fw_list_3' ) ) {
	function pixwell_rbc_fw_list_3( $attrs ) {
		...
		$settings['classes'] = 'fw-block fw-list-3';
		...
		if ( $query_data->have_posts() ) :
			...
			pixwell_rbc_fw_list_3_listing( $settings, $query_data );
 ...
 * fw list 1 listing
 */
if ( ! function_exists( 'pixwell_rbc_fw_list_3_listing' ) ) :
	function pixwell_rbc_fw_list_3_listing( $settings , $query_data ) {
			...
			while ( $query_data->have_posts() ) :
				...
				if ( ! empty( $settings['infeed_ad'] ) && ! wp_doing_ajax() ) :
					if ( ( ! empty( $settings['ad_pos_1'] ) && intval( $settings['ad_pos_1'] ) == $counter ) || ( ! empty( $settings['ad_pos_2'] ) && intval( $settings['ad_pos_2'] ) == $counter ) ) {
						echo '<div class="p-outer rb-col-m12 infeed-outer">';
				...
				echo '<div class="p-outer rb-col-m12">';
				pixwell_post_list_9( $settings );
				echo '</div>';
			endwhile;
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_list_3' ) ) {
	function pixwell_register_fw_list_3( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_list_3',
			'title'       => esc_html__( 'List Podcast Layout', 'pixwell' ),
			'description' => esc_html__( 'Display your podcast posts as a list (1 column) in the fullwidth section.', 'pixwell' ),
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-list-podcast.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_list_3', 30 );
