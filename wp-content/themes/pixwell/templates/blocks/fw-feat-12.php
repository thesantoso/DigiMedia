 ...
 * fullwide feat 12
 */
if ( ! function_exists( 'pixwell_rbc_fw_feat_12' ) ) {
	function pixwell_rbc_fw_feat_12( $attrs ) {
		...
		$settings['classes'] = 'fw-block fw-feat-12 none-margin';
		...
		if ( ! $query_data->have_posts() || $query_data->post_count < 5 ) {
			...
			echo '<div class="rb-owl carousel-feat-12 per-load">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				pixwell_post_grid_6( $settings );
				$counter ++;
			endwhile;
			echo '</div>';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_12' ) ) {
	function pixwell_register_fw_feat_12( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_12',
			'title'       => esc_html__( 'Food Carousel', 'pixwell' ),
			'tagline'     => esc_html__( 'Stretched', 'pixwell' ),
			'description' => esc_html__( 'Display your posts as a carousel (5 columns without gutter) in the fullwidth section.', 'pixwell' ),
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-12.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_12', 15 );
