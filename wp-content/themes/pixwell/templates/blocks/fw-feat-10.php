 ...
 * fullwidth feat 10
 */
if ( ! function_exists( 'pixwell_rbc_fw_feat_10' ) ) {
	function pixwell_rbc_fw_feat_10( $attrs ) {
		...
		$settings['classes'] = 'fw-block fw-feat-10 none-margin';
		...
		if ( ! $query_data->have_posts() || $query_data->post_count < 3 ) {
			...
			echo '<div class="rb-owl carousel-feat-10 per-load">';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_10' ) ) {
	function pixwell_register_fw_feat_10( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_10',
			'title'       => esc_html__( 'Blogger Carousel', 'pixwell' ),
			...
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-10.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_10', 12 );
