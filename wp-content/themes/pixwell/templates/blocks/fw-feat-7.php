 ...
 * fullwidth feat 7
 */
if ( ! function_exists( 'pixwell_rbc_fw_feat_7' ) ) {
	function pixwell_rbc_fw_feat_7( $attrs ) {
		...
		$settings['classes'] = 'fw-block fw-feat-7 none-margin';
		...
		if ( ! $query_data->have_posts() || $query_data->post_count < 4 ) {
			...
			echo '<div class="rb-owl carousel-feat-7 per-load">';
			while ( $query_data->have_posts() ) :
				$query_data->the_post();
				pixwell_post_overlay_6( $settings );
			endwhile;
			echo '</div>';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_7' ) ) {
	function pixwell_register_fw_feat_7( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_7',
			'title'       => esc_html__( 'LifeStyle Carousel', 'pixwell' ),
			'tagline'     => esc_html__( 'Wrapper', 'pixwell' ),
			'description' => esc_html__( 'Display your posts (overlay layout) as a carousel (3 columns without gutter) in the fullwidth section.', 'pixwell' ),
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-7.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_7', 13 );