 ...
 * fullwidth feat 5
 */
if ( ! function_exists( 'pixwell_rbc_fw_feat_5' ) ) {
	function pixwell_rbc_fw_feat_5( $attrs ) {
		...
		$settings['classes']       = 'fw-block fw-feat-5 none-margin';
		...
		if ( ! $query_data->have_posts() ) :
			...
			echo '<div class="rb-owl slider-feat-5 per-load">';
 ...
 */
if ( ! function_exists( 'pixwell_register_fw_feat_5' ) ) {
	function pixwell_register_fw_feat_5( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'fw_feat_5',
			'title'       => esc_html__( 'Slider', 'pixwell' ),
			...
			'section'     => 'fullwidth',
			'img'         => get_theme_file_uri( 'assets/images/fw-feat-5.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_fw_feat_5', 10 );