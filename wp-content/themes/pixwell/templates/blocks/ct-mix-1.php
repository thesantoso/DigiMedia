 ...
 * content mix 1
 */
if ( ! function_exists( 'pixwell_rbc_ct_mix_1' ) ) {
	function pixwell_rbc_ct_mix_1( $attrs ) {
		...
		$settings['classes'] = 'ct-block block-mix ct-mix-1 none-margin rb-col-m12 rb-col-d6';
		...
		if ( $query_data->have_posts() ) :
			...
			pixwell_rbc_ct_mix_1_listing( $settings, $query_data );
...
/**
 * content mix 1 listing
 */
if ( ! function_exists( 'pixwell_rbc_ct_mix_1_listing' ) ) :
	function pixwell_rbc_ct_mix_1_listing( $settings, $query_data ) {
		if ( method_exists( $query_data, 'have_posts' ) ) :
			...
			while ( $query_data->have_posts() ) :
				...
				if ( 1 == $flag ) {
					$flag = 0;
					pixwell_post_overlay_5( $settings );
				} else {
					pixwell_post_list_5( $settings );
				}
			endwhile;
		endif;
	}
 ...
 */
if ( ! function_exists( 'pixwell_register_ct_mix_1' ) ) {
	function pixwell_register_ct_mix_1( $blocks ) {
		...
		$blocks[] = array(
			'name'        => 'ct_mix_1',
			'title'       => esc_html__( 'Mix 1', 'pixwell' ),
			'tagline'     => esc_html__( '50% Width', 'pixwell' ),
			'description' => esc_html__( 'Display your posts as a mixed layout (grid overlay + small list). Recommended: add 2 blocks together to fill full-width the row.', 'pixwell' ),
			'section'     => 'content',
			'img'         => get_theme_file_uri( 'assets/images/ct-mix-1.png' ),
			'inputs'      => array(
				array(
					'type'        => 'category',
					'tab'         => 'filter',
...
add_filter( 'rbc_add_block', 'pixwell_register_ct_mix_1', 130 );
